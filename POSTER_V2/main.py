...
from models.PosterV2 import pyramid_trans_expr2
...
class AffectNetImageFolder(datasets.ImageFolder):
    def find_classes(self, directory):
        ...
        for c in classes:
            if c in class_mapping:
                class_to_idx[c] = class_mapping[c]
            elif c.lower() in class_mapping_lower:
                class_to_idx[c] = class_mapping_lower[c.lower()]
            else:
                raise ValueError(f"Unknown class directory: {c}")
...
def main():
    ...
    if args.evaluate is None:
        train_dataset = AffectNetImageFolder(
            traindir,
            transforms.Compose(
                [
    ...
    test_dataset = AffectNetImageFolder(
        valdir,
        transforms.Compose(
            [
                transforms.Resize((224, 224)),
    ...
    if args.evaluate is not None:
        if os.path.isfile(args.evaluate):
            print(f"=> loading checkpoint '{args.evaluate}'")
            ...
            print(f"=> no checkpoint found at '{args.evaluate}'")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        recorder.plot_curve(os.path.join("./output/", curve_name))
...
class RecorderMeter1:
    ...
    def plot_confusion_matrix(
        ...
        # Plot the raw confusion matrix passed to the function
        ...
        display_labels = (
            labels[:num_classes]
            if "labels" in globals() and len(labels) >= num_classes
            else [str(i) for i in range(num_classes)]
        )
        xlocations = np.array(range(len(display_labels)))
        plt.xticks(xlocations, display_labels, rotation=90)
        plt.yticks(xlocations, display_labels)
        plt.ylabel("True label")
        plt.xlabel("Predicted label")
        plt.show()

        # Compute and plot the normalized confusion matrix from y_true and y_pred
        y_true = self.y_true
        y_pred = self.y_pred
