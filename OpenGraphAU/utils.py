...
def draw_text(path, words, probs):
    ...
    for i, item in enumerate(range(21)):
        ...
        img = cv2.putText(
            img,
            AU_names[i] + " -- AU" + AU_ids[i] + f": {probs[i]:.2f}",
            (pos_x, y),
    ...
    for i, item in enumerate(range(21, 41)):
        ...
        img = cv2.putText(
            img,
            AU_names[item] + " -- AU" + AU_ids[item] + f": {probs[item]:.2f}",
            (pos_x_, y),
            cv2.FONT_HERSHEY_SIMPLEX,
            round(img.shape[1] / 2800, 3),
            color,
...
def BP4D_infolist(list):
    infostr = {
        f"AU1: {100.0 * list[0]:.2f} AU2: {100.0 * list[1]:.2f} AU4: {100.0 * list[2]:.2f} AU6: {100.0 * list[3]:.2f} AU7: {100.0 * list[4]:.2f} AU10: {100.0 * list[5]:.2f} AU12: {100.0 * list[6]:.2f} AU14: {100.0 * list[7]:.2f} AU15: {100.0 * list[8]:.2f} AU17: {100.0 * list[9]:.2f} AU23: {100.0 * list[10]:.2f} AU24: {100.0 * list[11]:.2f} "
    }
    return infostr
...
def hybrid_infolist(list):
    infostr = {
     ...
     AU22: {:.2f} AU23: {:.2f} AU24: {:.2f} AU25: {:.2f} AU26: {:.2f} AU27: {:.2f} AU32: {:.2f} AU38: {:.2f} AU39: {:.2f}\
      ...
      AUR10: {:.2f} AUL12: {:.2f} AUR12: {:.2f} AUL14: {:.2f} AUR14: {:.2f}".format(*[100.0 * x for x in list])
    }
    return infostr
...
def hybrid_prediction_infolist(pred, thresh):
    infostr_pred_probs = {
     ...
     AU22: {:.2f} AU23: {:.2f} AU24: {:.2f} AU25: {:.2f} AU26: {:.2f} AU27: {:.2f} AU32: {:.2f} AU38: {:.2f} AU39: {:.2f}\
      AUL1: {:.2f} AUR1: {:.2f} AUL2: {:.2f} AUR2: {:.2f} AUL4: {:.2f} AUR4: {:.2f} AUL6: {:.2f} AUR6: {:.2f} AUL10: {:.2f} \
      AUR10: {:.2f} AUL12: {:.2f} AUR12: {:.2f} AUL14: {:.2f} AUR14: {:.2f}".format(*[100.0 * x for x in pred])
    }
...
class image_eval:
    ...
    def __call__(self, img):
        normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        transform = transforms.Compose(
            [transforms.Resize(self.img_size), transforms.CenterCrop(self.crop_size), transforms.ToTensor(), normalize]
        )
        img = transform(img)
        return img
...
class WeightedAsymmetricLoss(nn.Module):
    def __init__(
        self, eps=1e-8, disable_torch_grad=True, weight=None, size_average=None, reduce=None, reduction="mean"
    ):
        super().__init__()
