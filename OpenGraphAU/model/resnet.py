...
def load_pretrained_weights(model, name):
    ...
    if state_dict is None:
        try:
            state_dict = torch.hub.load_state_dict_from_url(model_urls[name], model_dir=models_dir, map_location="cpu")
        except Exception as e:
            print(
                f"Warning: Failed to download/load pretrained weights for {name} from web: {e}. Proceeding with random initialization."
            )
            return model
