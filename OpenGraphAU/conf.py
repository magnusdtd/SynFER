...
# Datasets
parser.add_argument(
    "--dataset", default="hybrid", type=str, choices=["hybrid"], help="experiment dataset hybrid Dataset"
)

parser.add_argument(
    "--config", default="OpenGraphAU/config/hybrid_config.yaml", type=str, help="Path to the config file"
)
...
parser.add_argument(
    "-j", "--num_workers", default=4, type=int, metavar="N", help="number of data loading workers (default: 4)"
)
...
def set_outdir(conf):
    default_outdir = "outputs/results"
    if "timedir" in conf:
    ...
    mefl_path = os.path.join(current_dir, "model", "MEFL.py")
    if not os.path.exists(mefl_path):
        mefl_path = "./model/MEFL.py"
    anfl_path = os.path.join(current_dir, "model", "ANFL.py")
    if not os.path.exists(anfl_path):
        anfl_path = "./model/ANFL.py"
    shutil.copyfile(mefl_path, os.path.join(outdir, "MEFL.py"))
    shutil.copyfile(anfl_path, os.path.join(outdir, "ANFL.py"))
...
def set_logger(cfg):
    ...
    logging.info(f"writting logs to file {log_path}")
